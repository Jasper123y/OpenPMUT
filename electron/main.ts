...
function getIconPath(): string {
  ...
  const devPath = path.join(__dirname, '..', 'assets', 'icons', iconName);
  const prodPath = path.join(process.resourcesPath, 'assets', 'icons', iconName);
  if (IS_DEV && fs.existsSync(devPath)) return devPath;
...
function findPython(): string {
  ...
  if (process.env.OPENPMUT_PYTHON && fs.existsSync(process.env.OPENPMUT_PYTHON)) {
    return process.env.OPENPMUT_PYTHON;
  }
  // 2. Check conda env
  const condaPrefix = process.env.CONDA_PREFIX;
  if (condaPrefix) {
    const condaPy = path.join(condaPrefix, 'bin', 'python');
...
function doStartBackend(
  resolve: () => void,
  reject: (err: Error) => void
): void {
    const python = findPython();
    const backendDir = getPythonBackendPath();
    const backendApp = path.join(backendDir, 'app', 'main.py');
...
function pollBackendHealth(retries: number, delayMs: number): Promise<void> {
  return new Promise((resolve, reject) => {
...
function createMainWindow(): void {
  ...
  const rendererExists = require('fs').existsSync(path.join(rendererDir, 'index.html'));
      ...
      submenu: [
        {
          label: 'Documentation',
          click: () => {
            shell.openExternal('https://github.com/openpmut/openpmut');
          },
        },
        {
          label: 'Report Issue',
          click: () => {
            shell.openExternal('https://github.com/openpmut/openpmut/issues');
          },
        },
        { type: 'separator' },
        {
          ...
          click: () => {
            ...
            dialog.showMessageBox(mainWindow, {
              ...
              detail:
                'Standalone desktop application for Piezoelectric Micromachined ' +
  ...
  try {
    ...
    if (splashWindow) {
      splashWindow.webContents.executeJavaScript(
        `document.getElementById('status').textContent = 'Warning: Backend failed to start. Continuing...'`
      ).catch(() => {});
    }
